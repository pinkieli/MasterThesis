...
\end{align}
其中，$I$表示维度相容的单位矩阵。而Kronecker积定义如下，若$A\in\mathbf{R}^{m_1\times n_1}$和$B\in\mathbf{R}^{m_2\times n_2}$，则有
\begin{equation}
...
\end{bmatrix}\in\mathbf{R}^{m_1m_2\times n_1n_2}
\end{equation}
进一步，广义线性法可以被表达为如下形式
\begin{equation}
\begin{bmatrix}
...
\vartheta & \mathbf{v}^T & \mathbf{w}^T
\end{BMAT}
\end{equation}
...
\mathbf{A} & \mathbf{e-u} & \mathbf{u} & \mathbf{B}\\
\mathbf{v}^T & 1-\vartheta & \vartheta & \mathbf{w}^T\\
0 & 1 & 0 & 0\\
...
\end{bmatrix}
\end{equation}
其中，$\mathbf{I}$是维度为$s$的单位矩阵，而$\mathbf{0}$是带有合适维度的全零矩阵，$\mathbf{A}=[a_{ij}]_{s\times s},\mathbf{B}=[b_{ij}]_{s\times s},\mathbf{v}=[v_j]_{s\times 1},\mathbf{w}=[w_j]_{s\times 1}$，而$\mathbf{e}$为$s$维元素全为1的列向量。
...
\begin{equation}
Y^{[n]}=(\mathbf{B}\otimes \mathbf{I})Y^{[n-1]}+h(\mathbf{A}\otimes \mathbf{I})F(Y^{[n-1]})+h(\mathbf{R}\otimes \mathbf{I})F(Y^{[n]})
\end{equation}
