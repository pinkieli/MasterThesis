...
\end{equation}
称为线性多步法(\ref{eq:ch2lms})的特征多项式，其定义与经典线性多步法理论框架\cite{李寿佛2010,Butcher2008,Gear1971a,Hairer1993,ErnstHairer1996,Butcher1987,Fatunla1988,袁新鼎1987}下推导的结果是一致。
...
\begin{equation}
\bm{\xi}=\text{diag}(\xi_1,\cdots,\xi_s)=\text{diag}(h\xi(t_n+c_ih),\cdots,h\xi(t_n+c_sh))
\end{equation}
令
\begin{equation}
...
Runge-Kutta方法(\ref{eq:ch2RK})是AN稳定的，如果每当$c_i=c_j$时，对于所有的$\bm{\xi}=\text{diag}(\xi_1,\cdots,\xi_s)$使得$\xi_i=\xi_j$且有$\text{Re}(\xi_i)\le0,i=1,2,\cdots,s $都有函数$K(\xi)$满足
\begin{equation}
|K(\xi)|\le1
\end{equation}
\end{definition}
特别地，当$\bm{\xi}=z\bm{I}$时，函数$K(\xi)$将退化为Runge-Kutta方法的稳定函数(\ref{eq:ch2RKpwz})，因此可以得到
...
接着考虑如下的初值微分系统
\begin{equation}
y'(t)=g(t,y(t)),\quad y(0)=y_0\label{eq:ch2testfunBstable}
\end{equation}
其中，$g:\mathbb{R}\times \mathbb{R}^m\to\mathbb{R}^m$，且函数$g$满足单边Lipschitz条件\cite{Jackiewicz2009,Burrage1995}(单步Lipschitz常数为$v=0$)：
\begin{equation}
[g(t,y_1)-g(t,y_2)]^T(y_1-y_2)\le0
\end{equation}
$y_1,y_2\in\mathbb{R}^m$。令$y(t)$和$\tilde{y}(t)$分别表示系统(\ref{eq:ch2testfunBstable})在初值$y_0$和$\tilde{y}_0$下的解，则单步Lipschitz条件则表明了
\begin{equation}
||y(t_2)-\tilde{y}(t_2)||\le||y(t_1)-\tilde{y}(t_1)||,\quad\text{对于}\ 0\le t_1\le t_2\label{eq:ch2DisspativePro}
\end{equation}
于是称满足方程(\ref{eq:ch2DisspativePro})的微分系统(\ref{eq:ch2testfunBstable})是耗散的。

令$\{y_n\}_{n=0}^\infty$表示使用Runge-Kutta法(\ref{eq:ch2RK})求解系统(\ref{eq:ch2testfunBstable})在初值为$y_0$的数值解；而使用$\{\tilde{y}_n\}_{n=0}^\infty$表示使用Runge-Kutta法(\ref{eq:ch2RK})求解系统(\ref{eq:ch2testfunBstable})在初值为$\tilde{y}_0$的数值解。实质上，也可以理解为$\{\tilde{y}_{n=0}^\infty\}$为$\{y_n\}_{n=0}^\infty$在初值扰动下的数值解。基于上述理解，则可以引入B稳定和BN稳定性。
\begin{definition}[B稳定性]
Runge-Kutta方法(\ref{eq:ch2RK})是B稳定的\cite{Butcher1975,Fatunla1988}，如果对于求解满足\begin{equation}
[f(y_1)-f(y_2)]^T(y_1-y_2)\le0
\end{equation}的微分方程$y'(t)=f(y(t)),\ f:\mathbb{R}^m\to\mathbb{R}^m$的两个数值解$y_n,\tilde{y}_n$有
\begin{equation}
||y_{n+1}-\tilde{y}_{n+1}||\le ||y_{n}-\tilde{y}_{n}||,\quad h>0
\end{equation}成立。
\end{definition}
文\inlinecite{Butcher1975}中主要分析了隐式的Runge-Kutta方法满足B稳定性的要求和条件。同时，也知道了B稳定性是A稳定性对于非线性系统分析的扩展。
\begin{definition}[BN稳定性]
Runge-Kutta方法(\ref{eq:ch2RK})是BN稳定的\cite{Jackiewicz2009,Butcher2008,Butcher1975}，如果对于求解满足条件(\ref{eq:ch2DisspativePro})的微分方程(\ref{eq:ch2testfunBstable})有($n=0,1,2,\cdots$)
\begin{equation}
||y_{n+1}-\tilde{y}_{n+1}||\le ||y_{n}-\tilde{y}_{n}||,\quad h>0
\end{equation}
\end{definition}
