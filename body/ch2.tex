...
广义线性法($\bm{c},\bm{A},\bm{U},\bm{B},\bm{V}$)对给定的$z\in\mathbb{C}$是广义稳定的\cite{李寿佛2010}，如果对于这样的$z$，稳定矩阵$\bm{M}(z)$是幂有界的。
\end{definition}\vspace{-20pt}
...
广义线性法($\bm{c},\bm{A},\bm{U},\bm{B},\bm{V}$)是A稳定的\cite{Jackiewicz2009}，如果它的绝对稳定域包含复平面的左半平面，即
\begin{equation}
...
\end{equation}成立。
\end{definition}
一般情况下，A稳定是很困难或是不可能实现的，因此一个相对较弱的稳定性条件被提出来，即A$(\alpha)$稳定性。
\begin{definition}[A($\alpha$)稳定性]
广义线性法($\bm{c},\bm{A},\bm{U},\bm{B},\bm{V}$)是A($\alpha$)稳定的\cite{Butcher2008,李寿佛2010}，如果存在一个$\alpha\in(0,\pi)$有
\begin{equation}
S(\alpha)\triangleq\{z\in\mathbb{C}:|\arg(z)-\pi|<\alpha\}\subseteq\mathcal{A}
\end{equation}
其中，$\arg(z)$表示复数$z$的角度。
\end{definition}\vspace{-15pt}

另外可代替A稳定性的是刚性稳定\cite{Gear1971a}，其定义如下
\begin{definition}[刚性稳定的]
广义线性法($\bm{c},\bm{A},\bm{U},\bm{B},\bm{V}$)是刚性稳定的\cite{Gear1971a}，如果在复平面区域$R_1\triangleq\{z:\text{Re}(z)\le D\}$它是绝对稳定的；而在区域$R_2\triangleq\{z:D<\text{Re}(z)<\alpha,|\text{Im}(z)|<\theta\}$它是精确的。
\end{definition}\vspace{-15pt}
...
广义线性法($\bm{c},\bm{A},\bm{U},\bm{B},\bm{V}$)是L稳定的\cite{Butcher2008,李寿佛2010,Jackiewicz2009}，如果它是A稳定且是极端稳定的。
\end{definition}
