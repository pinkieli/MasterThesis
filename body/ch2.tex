...
为了计算广义线性阶条件的需要，下面需要定义两个B序列的复合。即
\begin{definition}
...
\end{equation}
\end{definition}
其中，$u\prec t$表示任意的与有根树$t$具有相同根的真子有根树$u$。而$t\backslash u$表示在有根树$t$中删除子树$u$之后剩下的子树。对于上述$ab(t)$的计算例子可以参见文献\inlinecite{Butcher1987a}。表\ref{Tab:ch2abt}给出了直到4阶有根树的$ab(t)$积运算结果。
\begin{table}[htbp]
\bicaption[Tab:ch2abt]{}{直到4阶有根树的初等权重函数$ab(t)$积}{Table$\!$}{Composition of elementary weight functions}
\vspace{0.5em}\centering\wuhao
\begin{tabular}{ccl}
\toprule[1.5pt]
有根树$t$& 注记 & $ab(t)$\\
...
显而易见的是，随着阶数的增加，其复合初等权重函数$ab(t)$的计算也越来越复杂。定义如下的两个集合
\begin{equation}
...
\end{equation}计算。
\end{theorem}
对于该定理的证明可以由对$i$的归纳法得之，亦可参见文献\inlinecite{Wright2002b}。
...
换句话说，广义线性法(\ref{eq:ch2GLM})的预相容、级相容和相容性条件由有根树理论表出时即是等式(\ref{eq:rtprecon})和(\ref{eq:rtconsist})应该被满足。
